...
def retrieve_with_velocity_estimation(
    ...
    # Best-parameter tracking: keep the iterate with the lowest cost
    best_cost = np.inf
    best_v_x = v_x
    best_z_T = z_T
    best_R = R
    ...
    for n in range(1, N_V + 1):
        ...
        if cost_current < best_cost:
            ...
            best_z_T = z_T
            best_R = R
            best_P_flat = P_flat.copy()

        # Step 2: Numerical gradients via central differences
        # Central differences eliminate the forward-bias that would otherwise
        # systematically push v_x in one direction.
        gamma_n = gamma_v / n

        # v_x gradient (central difference)
        cost_vp = _cost_for_params(v_x + dv, z_T, R, P_flat)
        cost_vm = _cost_for_params(v_x - dv, z_T, R, P_flat)
        grad_v = (cost_vp - cost_vm) / (2.0 * dv)
        # Clip to prevent runaway steps
        grad_v = np.clip(grad_v, -1e6, 1e6)
        v_x_new = v_x - gamma_n * grad_v

        # z_T gradient (central difference, optional)
        if estimate_z_T:
            gamma_z_n = gamma_z / n
            cost_zp = _cost_for_params(v_x, z_T + dz, R, P_flat)
            cost_zm = _cost_for_params(v_x, z_T - dz, R, P_flat)
            grad_z = (cost_zp - cost_zm) / (2.0 * dz)
            grad_z = np.clip(grad_z, -1e6, 1e6)
            z_T_new = z_T - gamma_z_n * grad_z
        else:
            z_T_new = z_T

        # R gradient (central difference, optional)
        if estimate_R:
            gamma_R_n = gamma_R / n
            cost_Rp = _cost_for_params(v_x, z_T, R + dR, P_flat)
            cost_Rm = _cost_for_params(v_x, z_T, R - dR, P_flat)
            grad_R = (cost_Rp - cost_Rm) / (2.0 * dR)
            grad_R = np.clip(grad_R, -1e6, 1e6)
        ...
        if verbose:
            params = f"v_x={v_x:.4f} (best={best_v_x:.4f}), z_T={z_T:.2f}"
            if estimate_R:
                params += f", R={R:.1f}"
            print(f"    outer iter {n:3d}: cost={cost_current:.6e}, {params}")

    # Revert to best parameters found across all iterations
    if best_P_flat is not None:
        v_x = best_v_x
        z_T = best_z_T
        R = best_R
        if verbose:
            print(f"\n  Reverted to best params: v_x={v_x:.4f}, z_T={z_T:.2f}, "
                  f"R={R:.1f}, cost={best_cost:.6e}")
